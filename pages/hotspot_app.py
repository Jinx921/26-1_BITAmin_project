...
st.markdown(
    ...
    <style>
      ...
      .block-container {padding-top: 3.5rem; padding-bottom: 1.2rem; max-width: 1280px;}
      ...
      .badge {padding:5px 10px; border-radius:999px; color:white; font-weight:700; font-size:0.86rem; display:inline-block;}
...
# APP
st.markdown(
    '<div class="title-wrap"><div class="title-main">서울 따릉이 고장 핫스팟 및 예측 지도</div><div class="small-muted" style="margin-top:12px;">현재 위험 지역과 미래 위험 지역을 한 번에 확인하세요.</div></div>',
    unsafe_allow_html=True,
)
...
st.markdown(
    ...
    <div class="section-header" style="margin-top:28px;">
      ...
      <div>
        <div class="section-title">핫스팟 요약</div>
        <div class="section-desc">현재 고장 현황 및 예측 요약</div>
      </div>
    </div>
    """,
    unsafe_allow_html=True,
...
k1.metric("현재 대여소 수", f"{len(hotspot_df):,}")
k2.metric("현재 총 고장건수", f"{int(hotspot_df['고장건수'].sum()):,}")
k3.metric(f"예측 {horizon}일 총 고장건수", f"{int(round(forecast_station_df['pred_fault_count'].sum())):,}")
k4.metric("예측 7일 고장 합", f"{int(round(next7_total)):,}")
...
with tab_now:
    st.markdown(
        ...
        <div class="section-header">
          <div class="section-icon section-icon-green">🗺️</div>
          <div>
            <div class="section-title">현재 고장 핫스팟 지도</div>
            <div class="section-desc">지도 원의 크기는 고장건수, 색상은 선택한 기준(위험등급/권역)을 의미합니다.</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    if now_f.empty:
        ...
        with l:
            st.markdown('<div class="sub-label">[상위 핫스팟 Top 20]</div>', unsafe_allow_html=True)
        ...
        with r:
            st.markdown('<div class="sub-label">[대표 고장유형 분포 (Top 10)]</div>', unsafe_allow_html=True)
...
with tab_pred:
    ...
    if not dates:
        ...
        if pred_f.empty:
            ...
            with l2:
                st.markdown('<div class="sub-label">[예측 상위 대여소 Top 20]</div>', unsafe_allow_html=True)
            ...
            with r2:
                st.markdown('<div class="sub-label">[예측 대표 고장유형 분포 (Top 10)]</div>', unsafe_allow_html=True)
            ...
            if forecast_global_df is not None and "date" in forecast_global_df.columns and "yhat_final" in forecast_global_df.columns:
                ...
                st.markdown('<div class="sub-label">[미래 전체 고장건수 추이]</div>', unsafe_allow_html=True)
